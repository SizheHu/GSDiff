...
sys.path.append('/root/autodl-tmp/gsdiff_topo-main')
sys.path.append('/root/autodl-tmp/gsdiff_topo-main/datasets')
sys.path.append('/root/autodl-tmp/gsdiff_topo-main/gsdiff_topo')
...
from gsdiff_topo.heterhouse_56_31 import TopoEdgeModel
...
device = 'cuda:0' # 本地
merge_points = True # 必须设成True否则后面的多边形提取的算法要推倒重写
align_points = True
aa_scale = 1
resolution = 512
...
for model_path_CDDPM in model_path_CDDPMs:
    if int(model_path_CDDPM[5 + len('outputs/structure-80-106-2/'):8 + len('outputs/structure-80-106-2/')]) % 1000 == 100: # model1000000.pt
        ...
        for batch_count in tqdm(range(batch_numbers)):
            ...
            bb_semantics_test_batch = bb_semantics_test_batch.to(device) # padding到8的低维房间语义（房间语义和房间类型是一码事，毕竟不是自然图像）
            bb_adjacency_matrix_test_batch = bb_adjacency_matrix_test_batch.to(device) # 拓扑图的邻接矩阵
            bb_semantics_padding_mask_test_batch = bb_semantics_padding_mask_test_batch.to(device) # 拓扑图的padding mask
            bb_global_matrix_test_batch = bb_global_matrix_test_batch.to(device)# 拓扑图的global_attn_matrix


            '''给出训练集拓扑图作为约束，拓扑图经过图嵌入网络获得拓扑图节点的嵌入，作为约束。
            注意其中有padding，被mask成了全0。 (batch_size_test, 8, 256)
