...
class HeterHouseModel(nn.Module):
    def __init__(self):
        ...
        self.semantics_embedding = nn.Linear(14, self.d_model)
    ...
    def forward(self, corners_withsemantics_t, global_attn_matrix, t):
        ...
        # print('corners_t[:, :, 0:1]', corners_t[:, :, 0:1])
        # print('corners_t[:, :, 0:1] * div_term[None, None, :].repeat(corners_t.shape[0], corners_t.shape[1], 1)', corners_t[:, :, 0:1] * div_term[None, None, :].repeat(corners_t.shape[0], corners_t.shape[1], 1))
        # print('(corners_t[:, :, 0:1] * div_term[None, None, :].repeat(corners_t.shape[0], corners_t.shape[1], 1)).sin()', (corners_t[:, :, 0:1] * div_term[None, None, :].repeat(corners_t.shape[0], corners_t.shape[1], 1)).sin())
        # assert 0
